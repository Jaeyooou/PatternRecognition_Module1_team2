...
def Find_Center_of_Gravity2(src):
    print("Find_Center_of_Gravity2 진입  , 5 7 구분용")
    gray = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY)
    ret, binary = cv2.threshold(gray, 180, 255, cv2.THRESH_BINARY)
    binary = cv2.bitwise_not(binary)
    contours = cv2.findContours(binary, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)[0]
...
def Find_heigth_width_rate(img):
    print("Find_height_width_rate ,1 과 2,3 구분 ")
...
def Find_top_Whitepx(img):
    print("Find_top_Whitepx , 1,2,3,5,7 구분 ")
    height, width = img.shape[0:2]
    img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    # 임계값 처리한 이미지 , 127 보다 높으면 255 , 아니면 0
    ret, imthres = cv2.threshold(img_gray, 127, 255, cv2.THRESH_BINARY_INV)  # binary image 로 변형
    ...
    for w in range(0, width):
        if imthres[0, w] == 255:
            cnt = cnt + 1
    white_area = cnt / width * 100
    print("top white px : " + str(white_area))
    if white_area > 70 :
        # 5,7 둘중 하나 임
        return True
    else:
...
if __name__ == '__main__':
         ...
         path= 'resource/test2/0.png' #이미지 입력
         src = cv2.imread(path)
     ...
     for i in range(0,30):
         ...
         if answer == j:# 정답이면
            print("정답 : 이미지는 : "+ str(i))
             ...
             print("오답 : 이미지는 "+ str(i) +"이며" , "제출한 답은:" + str(answer))
                #숫자의 윗면적이 70보다 낮으면
